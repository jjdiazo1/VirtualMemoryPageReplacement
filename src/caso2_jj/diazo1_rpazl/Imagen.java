...
public class Imagen {
    ...
    public void escribirImagen(String output) {
        ...
        try {
            ...
            for (int i = 0; i < alto; i++) {
                for (int j = 0; j < ancho; j++) {
                    pixel[0] = imagen[i][j][0]; // B
                    pixel[1] = imagen[i][j][1]; // G
                    pixel[2] = imagen[i][j][2]; // R
                    fos.write(pixel);
                }
     ...
     * @param longitud: tamaño del mensaje escondido y tamaño del vector (espacio disponible para almacenar información)
     * @pre la matriz imagen debe haber sido inicializada con una imagen. La imagen debe esconder el mensaje.
